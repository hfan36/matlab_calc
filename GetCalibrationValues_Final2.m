...
% load(strcat(folder_root, 'uvdata_new_extraction.mat'));

folder_root = 'H:\CT data\042414\';
load(strcat(folder_root, 'calibration_042414.mat'));
...
%Here there are 2 angles, rotates either ccw or cw, one of the 2 will work
%to calibration, it just depends on how the images were taken.
...
%% getting the standard deviation of all estimates

std_dx = std(result.dx);
std_dz = std(result.dz);
std_eta = std(result.eta);
std_phi = std(result.phi);
std_theta = std(result.theta);
std_R = std(result.R);
std_Rf_p = std(result.Rf_p);
std_x0Rf = std(result.x0_Rf);
std_y0Rf = std(result.y0_Rf);
std_z0Rf = std(result.z0_Rf);

fid = fopen(strcat(folder_root, 'Result_output.txt', 'w'));
fprintf(fid, '======= global parameters via Smekal''s method =========== \n');
fprintf(fid, 'R = %.4f \n', result.R_mean);
fprintf(fid, 'Rf = %.4f \n', min_Rf);
fprintf(fid, 'Ry = %.4f \n', result.Ry_p_mean);
fprintf(fid, 'Dx = %.4f \n', result.dx_mean);
fprintf(fid, 'Dz = %.4f \n', result.dz_mean);
fprintf(fid, 'xangle = %.4f \n', result.theta_mean);
fprintf(fid, 'yangle = %.4f \n', result.eta_mean);
fprintf(fid, 'zangle = %.4f \n', result.phi_mean);
fprintf(fid, '\n\n ======= object parameters=========== \n');
fprintf(fid, 'object theta = %.4f \n', min_theta);
fprintf(fid, 'object eta = %.4f \n', min_eta);
fprintf(fid, 'object phi = %.4f \n', min_phi);
fprintf(fid, 'min_dz = %.4f \n', min_dz);
fprintf(fid, '\n\n ======== standard deviations======== \n');
fprintf(fid, 'std R = %.4f \n', std_R);
fprintf(fid, 'std Dx = %.4f \n', std_dx);
fprintf(fid, 'std Dz = %.4f \n', std_dz);
fprintf(fid, 'std xangle = %.4f \n', std_theta);
fprintf(fid, 'std yangle = %.4f \n', std_eta);
fprintf(fid, 'std zangle = %.4f \n', std_phi);
% fprintf(fid, 'std Rf_p = %.4f \n', std_Rf_p);
fclose(fid);
